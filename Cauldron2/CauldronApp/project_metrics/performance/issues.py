...
def ttc_created_issues_bokeh(elastic, from_date, to_date):
    ...
    configure_figure(plot, 'https://gitlab.com/cauldronio/cauldron/'
                           '-/blob/master/guides/metrics/performance/ttc-created-issues.md')
    plot.title.text = 'Time to close (Issues created)'
    if len(timestamps) > 0:
...
def issues_still_open_by_creation_date_bokeh(elastic):
    ...
    plot = figure(x_axis_type="datetime",
                  ...
                  sizing_mode="stretch_width",
                  tools='')
    plot.title.text = '# Issues still open by creation date'
    configure_figure(plot, 'https://gitlab.com/cauldronio/cauldron/'
                           '-/blob/master/guides/metrics/performance/issues-still-open-by-creation-date.md')
...
def ttc_closed_issues_bokeh(elastic, from_date, to_date):
    """Generates a visualization showing the average and the median
    ...
    configure_figure(plot, 'https://gitlab.com/cauldronio/cauldron/'
                           '-/blob/master/guides/metrics/performance/ttc-closed-issues.md')
    plot.title.text = 'Time to close (Issues closed)'
    if len(timestamps) > 0:
        plot.x_range = Range1d(from_date - timedelta(days=1), to_date + timedelta(days=1))
...
def closed_created_issues_ratio_bokeh(elastic, from_date, to_date):
    """Generates a visualization showing the ratio between closed and
    created issues in a given date"""
    ...
    s.aggs.bucket('closed_issues', 'filter', Q('range', closed_at={'gte': from_date, "lte": to_date})) \
          .bucket('dates', 'date_histogram', field='closed_at', calendar_interval=interval_elastic)

    try:
        response = s.execute()
        creation_dates_buckets = response.aggregations.created_issues.dates.buckets
        closing_dates_buckets = response.aggregations.closed_issues.dates.buckets
    except ElasticsearchException as e:
        logger.warning(e)
        creation_dates_buckets = []
        closing_dates_buckets = []

    # Create the data structure for created issues
    issues_created, creation_dates_timestamps = [], []
    for date in creation_dates_buckets:
        creation_dates_timestamps.append(date.key)
        issues_created.append(date.doc_count)

    # Create the data structure for closed issues
    issues_closed, closing_dates_timestamps = [], []
    for date in closing_dates_buckets:
        closing_dates_timestamps.append(date.key)
        issues_closed.append(date.doc_count)

    creation_dates = pandas.DataFrame(list(zip(creation_dates_timestamps, issues_created)), columns =['timestamps', 'issues_created'])
    closing_dates = pandas.DataFrame(list(zip(closing_dates_timestamps, issues_closed)), columns =['timestamps', 'issues_closed'])

    data = pandas.merge(creation_dates, closing_dates, on='timestamps', how='outer', sort=True).fillna(0)

    data['ratio'] = data['issues_closed'] / data['issues_created']
    ...
    plot = figure(x_axis_type="datetime",
                  ...
                  sizing_mode="stretch_width",
                  tools='')
    plot.title.text = 'Issues closed / created ratio'
    configure_figure(plot, 'https://gitlab.com/cauldronio/cauldron/'
                           '-/blob/master/guides/metrics/performance/issues-closed-created-ratio.md')
    if not data.empty:
        plot.x_range = Range1d(from_date - timedelta(days=1), to_date + timedelta(days=1))
